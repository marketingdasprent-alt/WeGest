...
import { DadosPessoaisSection, CartaConducaoSection, DocumentosSection, SubmissaoSection } from './candidatura/sections';
...
export const CandidaturaFormulario: React.FC<CandidaturaFormularioProps> = ({ candidatura, onUpdate }) => {
  ...
  const currentDraftFields = () => ({
    nome, email, telefone, nif, morada, codigo_postal: codigoPostal, cidade,
    documento_tipo: documentoTipo, documento_numero: documentoNumero, documento_validade: documentoValidade,
    documento_ficheiro_url: documentoFicheiroUrl, documento_identificacao_verso_url: documentoIdentificacaoVersoUrl,
    carta_conducao: cartaConducao, carta_categorias: cartaCategorias, carta_validade: cartaValidade,
    carta_ficheiro_url: cartaFicheiroUrl, carta_conducao_verso_url: cartaConducaoVersoUrl,
    licenca_tvde_numero: licencaTvdeNumero, licenca_tvde_validade: licencaTvdeValidade,
    licenca_tvde_ficheiro_url: licencaTvdeFicheiroUrl, registo_criminal_url: registoCriminalUrl,
    comprovativo_morada_url: comprovativoMoradaUrl, comprovativo_iban_url: comprovativoIbanUrl,
  });
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (typeof draft.documento_validade === 'string') setDocumentoValidade(draft.documento_validade);
      if (typeof draft.documento_ficheiro_url === 'string') setDocumentoFicheiroUrl(draft.documento_ficheiro_url);
      if (typeof draft.documento_identificacao_verso_url === 'string') setDocumentoIdentificacaoVersoUrl(draft.documento_identificacao_verso_url);
      ...
      if (typeof draft.carta_conducao_verso_url === 'string') setCartaConducaoVersoUrl(draft.carta_conducao_verso_url);
      if (typeof draft.licenca_tvde_numero === 'string') setLicencaTvdeNumero(draft.licenca_tvde_numero);
      if (typeof draft.licenca_tvde_validade === 'string') setLicencaTvdeValidade(draft.licenca_tvde_validade);
      if (typeof draft.licenca_tvde_ficheiro_url === 'string') setLicencaTvdeFicheiroUrl(draft.licenca_tvde_ficheiro_url);
      if (typeof draft.registo_criminal_url === 'string') setRegistoCriminalUrl(draft.registo_criminal_url);
      if (typeof draft.comprovativo_morada_url === 'string') setComprovativoMoradaUrl(draft.comprovativo_morada_url);
      if (typeof draft.comprovativo_iban_url === 'string') setComprovativoIbanUrl(draft.comprovativo_iban_url);
      toast({ title: 'Rascunho recuperado', description: 'Recuperámos os dados que tinha preenchido neste dispositivo.' });
    } catch (e) { console.error('Erro ao restaurar rascunho local:', e); }
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(draftKey, JSON.stringify({ ...fields, _savedAt: new Date().toISOString() }));
    } catch (e) { console.error('Erro ao guardar rascunho local:', e); }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [draftKey, nome, email, telefone, nif, morada, codigoPostal, cidade,
    documentoTipo, documentoNumero, documentoValidade, documentoFicheiroUrl, documentoIdentificacaoVersoUrl,
    cartaConducao, cartaCategorias, cartaValidade, cartaFicheiroUrl, cartaConducaoVersoUrl,
  ...
  const saveUploadUrlToDb = async (column: string, url: string) => {
    ...
    try {
      await supabase.from('motorista_candidaturas')
        .update({ [column]: url || null } as TablesUpdate<'motorista_candidaturas'>)
        .eq('id', candidatura.id);
      onUpdate();
    } catch (err) { console.error('Erro ao persistir URL do ficheiro na BD:', err); }
  };

  const clearLocalDraft = () => {
    if (draftKey) try { localStorage.removeItem(draftKey); } catch (e) { console.error('Erro ao limpar rascunho local:', e); }
  };

  const buildCandidaturaData = () => ({
    user_id: user!.id, nome, email, telefone: telefone || null, nif: nif || null,
    morada: morada || null, codigo_postal: codigoPostal || null, cidade: cidade || null,
    documento_tipo: documentoTipo || null, documento_numero: documentoNumero || null,
    documento_validade: documentoValidade || null, documento_ficheiro_url: documentoFicheiroUrl || null,
    documento_identificacao_verso_url: documentoIdentificacaoVersoUrl || null,
    carta_conducao: cartaConducao || null, carta_categorias: cartaCategorias.length > 0 ? cartaCategorias : null,
    carta_validade: cartaValidade || null, carta_ficheiro_url: cartaFicheiroUrl || null,
    carta_conducao_verso_url: cartaConducaoVersoUrl || null,
    licenca_tvde_numero: licencaTvdeNumero || null, licenca_tvde_validade: licencaTvdeValidade || null,
    licenca_tvde_ficheiro_url: licencaTvdeFicheiroUrl || null,
    registo_criminal_url: registoCriminalUrl || null, comprovativo_morada_url: comprovativoMoradaUrl || null,
    comprovativo_iban_url: comprovativoIbanUrl || null, observacoes: observacoes || null,
  });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (candidatura) {
        const { data: rows, error } = await supabase.from('motorista_candidaturas').update(data).eq('id', candidatura.id).select('id');
        if (error) throw error;
        if (!rows || rows.length === 0) throw new Error('sem_permissao_guardar');
      } else {
        const { error } = await supabase.from('motorista_candidaturas').insert({ ...data, status: 'rascunho' });
        if (error) throw error;
      }
      ...
      toast({ title: 'Erro', description: traduzirErro(error.message, 'Ocorreu um erro ao guardar.'), variant: 'destructive' });
    } finally { setSaving(false); }
  };

  const scrollToFirstError = () => {
    setTimeout(() => { document.querySelector('[data-field-error="true"]')?.scrollIntoView({ behavior: 'smooth', block: 'center' }); }, 60);
  };

  const handleSubmit = async () => {
    const campos: CandidaturaCampos = {
      nome, email, telefone, nif, morada, cidade, codigoPostal,
      documentoTipo, documentoNumero, documentoValidade, documentoFicheiroUrl, documentoIdentificacaoVersoUrl,
      cartaConducao, cartaCategorias, cartaValidade, cartaFicheiroUrl, cartaConducaoVersoUrl,
      licencaTvdeNumero, licencaTvdeValidade, licencaTvdeFicheiroUrl,
      registoCriminalUrl, comprovativoMoradaUrl, comprovativoIbanUrl,
    ...
    if (problemas.length > 0) {
      ...
      toast({
        title: problemas.length === 1 ? '1 ponto por corrigir antes de submeter' : `${problemas.length} pontos por corrigir antes de submeter`,
        description: (<div className="mt-1 space-y-1">
          <p>Corrija o seguinte (também assinalado a vermelho no formulário):</p>
          <ul className="list-disc space-y-0.5 pl-4">
            {visiveis.map((msg, i) => <li key={i}>{msg}</li>)}
    ...
    setSubmitting(true);
    try {
      const data = { ...buildCandidaturaData(), status: 'submetido', data_submissao: new Date().toISOString() } as TablesInsert<'motorista_candidaturas'>;
      if (candidatura) {
        const { data: rows, error } = await supabase.from('motorista_candidaturas').update(data).eq('id', candidatura.id).select('id');
        if (error) throw error;
        if (!rows || rows.length === 0) throw new Error('sem_permissao_guardar');
      ...
      clearLocalDraft();
      toast({ title: 'Candidatura Submetida!', description: 'Os seus documentos serão analisados pela nossa equipa.' });
      onUpdate();
    } catch (error: any) {
      console.error('Erro ao submeter:', error);
      toast({ title: 'Erro', description: traduzirErro(error.message, 'Ocorreu um erro ao submeter a candidatura.'), variant: 'destructive' });
    } finally { setSubmitting(false); }
  };

  const completionPercentage = () => {
    const fields = [nome, email, telefone, nif, morada, cidade, documentoTipo, documentoNumero, documentoValidade,
      documentoFicheiroUrl, documentoIdentificacaoVersoUrl, cartaConducao, cartaCategorias.length > 0, cartaValidade,
      cartaFicheiroUrl, cartaConducaoVersoUrl, licencaTvdeNumero, licencaTvdeValidade, licencaTvdeFicheiroUrl,
      registoCriminalUrl, comprovativoMoradaUrl, comprovativoIbanUrl];
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-4xl mx-auto px-4 pb-8 space-y-6">
        ...
        <DadosPessoaisSection
          campos={{ nome, email, telefone, nif, morada, cidade, codigoPostal, comprovativoMoradaUrl }}
          setNome={setNome} setEmail={setEmail} setTelefone={setTelefone} setNif={setNif}
          setMorada={setMorada} setCidade={setCidade} setCodigoPostal={setCodigoPostal}
          setComprovativoMoradaUrl={setComprovativoMoradaUrl}
          fieldErrors={fieldErrors} setFieldError={setFieldError} clearFieldError={clearFieldError}
          onUploadToDb={saveUploadUrlToDb}
        />

        <CartaConducaoSection
          documentoTipo={documentoTipo} documentoNumero={documentoNumero} documentoValidade={documentoValidade}
          documentoFicheiroUrl={documentoFicheiroUrl} documentoIdentificacaoVersoUrl={documentoIdentificacaoVersoUrl}
          setDocumentoTipo={setDocumentoTipo} setDocumentoNumero={setDocumentoNumero} setDocumentoValidade={setDocumentoValidade}
          setDocumentoFicheiroUrl={setDocumentoFicheiroUrl} setDocumentoIdentificacaoVersoUrl={setDocumentoIdentificacaoVersoUrl}
          cartaConducao={cartaConducao} cartaCategorias={cartaCategorias} cartaValidade={cartaValidade}
          cartaFicheiroUrl={cartaFicheiroUrl} cartaConducaoVersoUrl={cartaConducaoVersoUrl}
          setCartaConducao={setCartaConducao} setCartaCategorias={setCartaCategorias} setCartaValidade={setCartaValidade}
          setCartaFicheiroUrl={setCartaFicheiroUrl} setCartaConducaoVersoUrl={setCartaConducaoVersoUrl}
          fieldErrors={fieldErrors} setFieldError={setFieldError} clearFieldError={clearFieldError}
          onUploadToDb={saveUploadUrlToDb}
        />

        <DocumentosSection
          licencaTvdeNumero={licencaTvdeNumero} licencaTvdeValidade={licencaTvdeValidade} licencaTvdeFicheiroUrl={licencaTvdeFicheiroUrl}
          setLicencaTvdeNumero={setLicencaTvdeNumero} setLicencaTvdeValidade={setLicencaTvdeValidade} setLicencaTvdeFicheiroUrl={setLicencaTvdeFicheiroUrl}
          registoCriminalUrl={registoCriminalUrl} comprovativoIbanUrl={comprovativoIbanUrl}
          setRegistoCriminalUrl={setRegistoCriminalUrl} setComprovativoIbanUrl={setComprovativoIbanUrl}
