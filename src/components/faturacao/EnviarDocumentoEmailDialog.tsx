...
import { enviarDocumentoFiscalEmail } from '@/lib/emailDocumentoFiscal';

interface Props {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  /** Documento fiscal a enviar (o PDF é obtido on-demand). */
  invoice: InvoiceMetadata | null;
  /** Ex.: "Contrato #0703" ou "Reserva #12". */
  contextoLabel: string;
  /** Cliente e/ou condutor resolvidos do contrato/reserva. */
  entidades: ContactoEntidade[];
}

export function EnviarDocumentoEmailDialog({
  open,
  onOpenChange,
  invoice,
  contextoLabel,
  ...
  // (Re)inicializa os campos sempre que o dialog abre para um documento — por
  // defeito na entidade "Cliente" com o respetivo email (editável).
  useEffect(() => {
    if (!open) return;
  ...
  async function handleEnviar() {
    ...
    setEnviando(true);
    try {
      await enviarDocumentoFiscalEmail({
        to: dest,
        toNome: entidadeSelecionada?.nome,
        subject: assuntoDocumento(invoice, contextoLabel, idioma),
        mensagem,
        invoice,
      });
  ...
  const tituloDoc = invoice
    ? `${tipoDocumentoLabel(invoice.tipo, idioma)}${invoice.numero ? ` ${invoice.numero}` : ''}`
    : 'Documento';
    ...
    <Dialog open={open} onOpenChange={(o) => (enviando ? undefined : onOpenChange(o))}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {tituloDoc}
            {contextoLabel ? ` · ${contextoLabel}` : ''} — envia o PDF em anexo para o cliente ou
            condutor.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleEnviar} disabled={enviando || !email.trim()}>
            {enviando ? (
