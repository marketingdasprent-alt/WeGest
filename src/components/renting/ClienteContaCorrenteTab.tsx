...
import { Banknote, Receipt, Scale, Download, Loader2, FileText } from 'lucide-react';
...
import { DOC_TIPO_LABEL, DOC_TIPO_CLASS, type FaturacaoRow } from '@/components/administrativo/faturacao';
...
/** Cartão de resumo no topo (label + valor grande + ícone em pill). */
function ResumoCard({
  label,
  valor,
  icon,
...
export function ClienteContaCorrenteTab({ clienteId }: ClienteContaCorrenteTabProps) {
  ...
  async function baixarPdf(e: React.MouseEvent, inv: InvoiceMetadata) {
    ...
    try {
      ...
      toast.error(`Erro ao obter PDF: ${err?.message ?? 'indisponível'}`);
    } finally {
      setBaixandoId(null);
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {saldo > 0.005 && !isLoading && (
        <p className="text-xs text-muted-foreground">
          O cliente tem <span className="font-semibold text-red-600 dark:text-red-400">
      ...
      {/* Tabela de documentos / movimentos */}
                  ...
                  >
                    ...
                    <TableCell className="text-center">
                      {inv ? (
                        ...
                        <FileText className="h-3.5 w-3.5 mx-auto text-muted-foreground/30" />
                      )}
                    </TableCell>
                  </TableRow>
                );
