...
const Contatos = () => {
  ...
  // Mantém a mesma regra que a mensagem de vazio já usava: qualquer filtro fora
  // do valor por omissão conta. Extraída para o vazio poder oferecer a acção
  // certa em vez de só constatar que não há nada.
  const temFiltrosAtivos = Object.values(filters).some((f) =>
    Array.isArray(f) ? f.length > 0 : Boolean(f) && f !== 'todos'
  );
  ...
  if (loading && currentPage === 1) {
    return (
      <div className="min-h-screen bg-background p-6">
        <TableSkeleton colunas={5} linhas={8} />
      </div>
    );
  }
