...
export function useSaveCamposDinamicos() {
  ...
  return useMutation({
    mutationFn: async (campos: CampoEfetivo[]) => {
      ...
      const { error: insErr } = await supabase.from('org_campos_dinamicos').insert(rows);
...
export function useCriarCampoCatalogo() {
  ...
  return useMutation({
    mutationFn: async (campo: Omit<CampoCatalogoCustom, 'id'>) => {
      const { error } = await supabase.from('campos_catalogo').insert({
        chave: campo.chave,
        label: campo.label,
...
export function useApagarCampoCatalogo() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('campos_catalogo').delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: CATALOGO_KEY });
