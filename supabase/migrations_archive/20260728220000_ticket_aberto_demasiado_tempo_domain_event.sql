-- Item 9/13 da lista do chefe: "Alerta de reparação aberta há demasiado
-- tempo". Não existe nenhum conceito de SLA/prazo no sistema (nem em
-- assistencia_categorias, nem em prioridade, nem data_estimada — esta
-- última tão pouco preenchida quanto viatura_documentos.data_validade
-- foi no item 5). Limiar fixo escolhido pelo utilizador: 7 dias desde
-- created_at sem o ticket estar resolvido/fechado, mesmo padrão scan
-- diário de emit_candidaturas_paradas_events() (item 3).

create or replace function public.emit_tickets_atrasados_events()
returns void
language plpgsql
security definer
...
begin
  ...
  select
    ...
    'cron'
  from public.assistencia_tickets t
  join public.viaturas v on v.id = t.viatura_id
  where t.status not in ('resolvido', 'fechado')
    and t.org_id is not null
    and t.created_at <= now() - interval '7 days'
    and not exists (
      select 1 from public.domain_events e
      where e.entity_table = 'assistencia_tickets'
        and e.entity_id = t.id
        and e.event_type = 'assistencia_ticket.aberto_demasiado_tempo'
        and e.processed_at is null
    );
end;
$$;

revoke all on function public.emit_tickets_atrasados_events() from public, anon, authenticated;
grant execute on function public.emit_tickets_atrasados_events() to service_role;

select cron.schedule(
  'automation-emit-tickets-atrasados-diario',
  '0 8 * * *',
