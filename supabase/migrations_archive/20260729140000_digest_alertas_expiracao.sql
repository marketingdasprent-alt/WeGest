-- Incidente real (28/07): viatura.seguro_expirando mandou 446 emails num
-- dia a 17 pessoas (83 viaturas diferentes atingiram o limiar de "seguro a
-- expirar" ao mesmo tempo) — o MESMO padrão do incidente de
-- contrato_renting.renovacao_proxima (ver 20260727360000_digest_diario_email.sql),
-- que já foi corrigido com modo digest. As outras 7 regras de alerta em
-- massa (viatura.seguro/inspecao/extintor/iuc/manutencao_preventiva a
-- expirar, motorista.carta/licenca_tvde a expirar) nunca receberam o mesmo
-- tratamento e ficaram expostas ao mesmo risco. enviar_digests_diarios()
-- já é genérica (lê enviar_email_digest de qualquer regra), por isso esta
-- correção só precisa de ligar a flag — sem código novo.

update public.automation_rules
set acao_config = jsonb_set(acao_config, '{enviar_email_digest}', 'true'::jsonb)
where event_type in (
