...
--
-- Nota 15/07: a versão em produção evoluiu entre o planeamento desta
-- migration e a sua aplicação — ganhou parâmetros p_km_inicio/p_km_fim
-- (regista quilometragem na renovação, actualiza viaturas.km_atual, e
-- lança um extra automático "Km excedente" se ultrapassar kms_incluidos).
-- Esta migration foi reconstruída em cima dessa versão actual (confirmada
-- via pg_get_functiondef em produção antes de aplicar) — preserva toda a
-- lógica de km, só remove o guard de regime.
--
...
--
-- Testado em transacção com ROLLBACK contra um contrato TVDE real
-- (df61e494-6bb9-4a7f-b905-66da8d693af4) antes de aplicar: renovou
-- correctamente (versao 1→2, data_inicio = data_fim antigo, data_fim
-- recalculado +30 dias), sem deixar nenhum dado de teste na BD.
-- ============================================================

CREATE OR REPLACE FUNCTION public.renovar_contrato_renting(p_contrato_id uuid, p_km_inicio integer DEFAULT NULL::integer, p_km_fim integer DEFAULT NULL::integer)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
