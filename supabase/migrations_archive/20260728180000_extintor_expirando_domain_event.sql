-- Item 5/13 da lista do chefe: "Alerta documentos da viatura a expirar".
-- viaturas.extintor_validade tem dados reais e não estava coberto pelo
-- motor novo (só por um cron/edge function antigo, entretanto a ler a
-- coluna errada depois da migração 20260423174900_move_extintor_to_viaturas.sql
-- ter mudado extintor de motorista_viaturas para viaturas — deixado como
-- está, fora de scope). A tabela genérica viatura_documentos
-- (DUA/carta verde/IPO/etc.) tem data_validade 0% preenchida em produção
-- — decisão explícita de não a cobrir agora, sem dados reais para alertar.
  ...
  and (
    ...
    or (
      tipo = any (array[
        ...
        'contrato_renting_sem_checkin', 'viatura_extintor_expirando'
      ])
