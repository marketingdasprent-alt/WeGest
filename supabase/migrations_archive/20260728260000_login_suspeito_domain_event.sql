-- Item 13/13 (último) da lista do chefe: "Deteção de tentativas de
-- login suspeitas". Login é 100% client-side via
-- supabase.auth.signInWithPassword (src/pages/Login.tsx) — não existe
-- nenhum rasto de tentativas hoje, e auth.audit_log_entries (GoTrue)
-- não é acessível por trigger a partir do schema public. Solução:
-- tabela nova login_attempts, preenchida pelo próprio formulário de
-- login (sucesso e falha), e um scan de alta frequência (5 em 5 min,
-- ao contrário dos scans diários dos outros itens, porque isto é
-- sensível ao tempo) que deteta 5+ tentativas falhadas para o mesmo
-- email em 15 minutos. Notifica só os Administradores da organização
-- visada (destinatarios_cargo_ids vazio — a mesma condição
-- "uo.is_admin = true" do motor genérico já garante isto, sem
...
create policy "qualquer um pode registar uma tentativa de login"
on public.login_attempts for insert
to anon, authenticated
with check (true);

create policy "só admins veem tentativas de login"
...
select cron.schedule(
  'automation-emit-login-suspeito',
  '*/5 * * * *',
...
alter table public.notificacoes add constraint notificacoes_tipo_check check (tipo = any (array[
  ...
  'motorista_ficha_incompleta', 'invoice_nao_enviada_ao_cliente', 'seguranca_login_suspeito'
]));
