-- Item 3/13 da lista do chefe: "Candidatura de novo motorista parada para
-- ser aceite". Hoje existe notificar_motorista_pendente() (trigger em
-- motorista_candidaturas), mas esse aviso desaparece assim que alguém
-- ABRE a candidatura (status muda para em_analise) — mesmo que ninguém
-- decida nada depois. Este alerta fecha esse buraco: scan diário (mesmo
-- padrão de emit_expiry_events()) para candidaturas em 'submetido' OU
-- 'em_analise' há mais de 3 dias sem decisão final.
...
begin
  ...
  where c.status in ('submetido', 'em_analise')
...
select cron.schedule(
  'automation-emit-candidaturas-paradas-diario',
  '0 8 * * *',
  $$select public.emit_candidaturas_paradas_events()$$
);

-- Regra + template novos (seed_automacao_defaults) — mesmo default
-- (cargo Gestor TVDE) dos 2 itens anteriores.
...
alter table public.notificacoes add constraint notificacoes_tipo_check check (tipo = any (array[
  ...
  'contrato_renting_criado', 'motorista_candidatura_parada'
]));
...
-- execute_automation_runs(): mapeia o novo event_type + link.
