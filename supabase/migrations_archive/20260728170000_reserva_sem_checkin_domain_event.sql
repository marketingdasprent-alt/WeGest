-- Item 4/13 da lista do chefe: "Reservas sem Checkin". Confirmado que
-- "Checkin" aqui é a DEVOLUÇÃO do carro (fim do contrato) — na app isto
-- já se chama "Check-in" (RecolhasPendentesDrawer.tsx), o oposto de
-- "entrega"/"Check-out" (início). O estado vive no contrato
-- (contratos_renting.estado_operacional + calendario_eventos tipo
-- 'recolha'/'devolucao'/'troca'), não na reserva em si — reserva.estado
-- já passa a 'em_curso' assim que o contrato nasce, muito antes da
-- devolução real.
--
-- Scan diário (mesmo padrão de emit_expiry_events()/
-- emit_candidaturas_paradas_events()): contrato com data_fim passada,
-- ainda 'em_curso' (não devolvido/cancelado), e o evento de
-- recolha/devolução/troca ainda sem realizado_em.
...
begin
  ...
  join public.calendario_eventos ce
    on ce.origem_tipo = 'contrato_renting'
...
select cron.schedule(
  'automation-emit-reservas-sem-checkin-diario',
  '0 8 * * *',
...
-- execute_automation_runs(): mapeia o novo event_type (link já existe:
-- entity_table='contratos_renting' → /renting/contratos/<id>).
