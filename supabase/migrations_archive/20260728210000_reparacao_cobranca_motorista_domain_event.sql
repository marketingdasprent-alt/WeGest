-- Item 8/13: "Notificação ao motorista quando uma reparação fecha com valor a cobrar"
-- Dispara quando é criado um lançamento de débito (categoria='reparacao') em
-- motorista_financeiro — o momento exato, com o valor já confirmado, em que
-- useTicketClosure.ts regista a cobrança ao motorista.
-- Destinatário é o próprio motorista (novo destinatarios_estrategia='motorista'),
-- não um cargo/gestor: notificação in-app se tiver conta no portal
-- (motoristas_ativos.user_id) + email sempre para motoristas_ativos.email.

create or replace function public.fn_motorista_financeiro_reparacao_domain_event()
returns trigger
language plpgsql
security definer
...
declare
  ...
  v_ticket_numero int;
begin
  if new.categoria <> 'reparacao' or new.tipo <> 'debito' or new.reparacao_id is null then
    return new;
  end if;

  select ma.org_id into v_org_id
  from public.motoristas_ativos ma
  where ma.id = new.motorista_id;

  select v.matricula into v_matricula
  from public.viatura_reparacoes vr
  join public.viaturas v on v.id = vr.viatura_id
  where vr.id = new.reparacao_id;

  select at.numero into v_ticket_numero
  from public.assistencia_tickets at
  where at.reparacao_id = new.reparacao_id
  limit 1;
...
drop trigger if exists trg_motorista_financeiro_reparacao_domain_event on public.motorista_financeiro;
create trigger trg_motorista_financeiro_reparacao_domain_event
after insert on public.motorista_financeiro
for each row
execute function public.fn_motorista_financeiro_reparacao_domain_event();
...
begin
  ...
  loop
    begin
      ...
      -- Destinatário é o próprio motorista (não um cargo/gestor) — o motorista pode
      -- nem estar em user_organizacoes, por isso resolve-se e trata-se à parte,
      -- sem passar pelo loop genérico scoped a org_id/cargo abaixo.
