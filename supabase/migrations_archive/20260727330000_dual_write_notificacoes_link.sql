-- Fix: as notificações da dupla escrita (viatura.seguro_expirando,
-- viatura.inspecao_expirando, motorista.carta_expirando,
-- motorista.licenca_tvde_expirando) não preenchiam `link` nem
-- `viatura_id` em `notificacoes` — o botão "Ver" caía sempre no default
-- de candidaturas (notificacaoLink/notificacaoLabel em
-- src/utils/notificacoes.ts), mesmo sendo sobre uma viatura ou motorista
-- concretos. Passa a computar o destino a partir de entity_table/entity_id
-- do próprio automation_run.
--
-- Nota: notificações já criadas antes deste fix não têm como ser
-- corrigidas retroativamente (não guardavam entity_id nenhum) — só as
-- novas, a partir de agora, ficam com link correto.
