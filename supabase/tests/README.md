...
| `dual_write_notificacoes_legado.test.sql` | Motor de Automação: `execute_automation_runs()` escreve também em `notificacoes` (tabela antiga, lida pelo sino/popup real) para os 4 event_types com `tipo` mapeado — whitelist explícita: um event_type fora da lista não escreve em `notificacoes` nem falha o run. |
