...
| `seed_automacao_defaults.test.sql` | Motor de Automação: `seed_automacao_defaults()` — cria as 5 regras por-omissão (seguro/IPO/carta/licença TVDE/cobrança gerada) para uma org, idempotente, e uma organização nova recebe-as automaticamente via trigger. |
...
| `dual_write_notificacoes_legado.test.sql` | Motor de Automação: `execute_automation_runs()` escreve também em `notificacoes` (tabela antiga, lida pelo sino/popup real) para os 5 event_types com `tipo` mapeado (incluindo `cobranca.gerada`) — whitelist explícita: um event_type fora da lista não escreve em `notificacoes` nem falha o run. |
...
| `contrato_alterado.test.sql` | Motor de Automação: um UPDATE em `contratos` (estado/motorista/viatura/empresa/datas/km/documento) publica `contrato.alterado` com a lista das alterações e uma mensagem legível; combustível não publica; a organização nova nasce com as regras gémeas notificação+email para o Administrador; a cadeia chega ao sino e à fila de email do Administrador e não a outro cargo; uma segunda alteração não é suprimida como «aviso em aberto». |
