...
insert into public.notifications (id, org_id, destinatario_user_id, template_codigo, titulo) values
  ('00000000-0000-0000-0000-000000n1e001', '00000000-0000-0000-0000-0000000a0000', '00000000-0000-0000-0000-0000000a0001', 'teste.notif', 'Notificação de Teste');

insert into public.notification_queue (id, notification_id, org_id, canal, destinatario, template_codigo, max_attempts) values
  ('00000000-0000-0000-0000-000000q1e001', '00000000-0000-0000-0000-000000n1e001', '00000000-0000-0000-0000-0000000a0000', 'email', 'a1@notif-queue.pt', 'teste.notif', 2);
...
select is(
  (select status from public.notification_queue where id = '00000000-0000-0000-0000-000000q1e001'),
  'running',
...
select public.notification_queue_fail('00000000-0000-0000-0000-000000q1e001', 'erro smtp 1');
...
select ok(
  (select next_attempt_at > now() from public.notification_queue where id = '00000000-0000-0000-0000-000000q1e001'),
  'falha agenda o próximo attempt no futuro'
);

-- 7. Esgotar tentativas (max_attempts=2): dead-letter.
update public.notification_queue set next_attempt_at = now() where id = '00000000-0000-0000-0000-000000q1e001';
select public.notification_queue_claim('email', 10);
select public.notification_queue_fail('00000000-0000-0000-0000-000000q1e001', 'erro smtp 2');
...
select is(
  (select count(*)::int from public.failed_jobs where source_table = 'notification_queue' and source_id = '00000000-0000-0000-0000-000000q1e001'),
  1,
...
insert into public.notification_queue (id, notification_id, org_id, canal, destinatario, template_codigo) values
  ('00000000-0000-0000-0000-000000q2e001', '00000000-0000-0000-0000-000000n1e001', '00000000-0000-0000-0000-0000000a0000', 'email', 'a1@notif-queue.pt', 'teste.notif');

select public.notification_queue_claim('email', 10);
select public.notification_queue_complete('00000000-0000-0000-0000-000000q2e001');

select is(
  (select status from public.notification_queue where id = '00000000-0000-0000-0000-000000q2e001'),
  'sent',
