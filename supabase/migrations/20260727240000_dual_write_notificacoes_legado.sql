-- Motor de Automação — fecha o item do MVP "notifications nova a correr em
-- paralelo com notificacoes (dupla escrita), sem quebrar o popup/página
-- atuais". Até agora o pipeline só escrevia em `notifications` (nova) —
-- o sino/popup real da aplicação lê exclusivamente de `notificacoes`
-- (antiga, ver src/hooks/useNotificacoes.ts), pelo que nenhum utilizador
-- via alguma vez estas notificações fora da dashboard admin.
--
-- Estratégia: dupla escrita apenas para os 4 tipos de evento hoje
-- seedados (seguro/IPO/carta/licença TVDE). `tipo` em notificacoes é um
-- CHECK-list fechado (já causou perda silenciosa de 'recibo_anulado' no
-- passado) — o mapeamento abaixo é uma whitelist explícita: um
-- event_type desconhecido simplesmente não gera escrita em notificacoes
-- (cai em NULL), nunca rebenta a função toda. Isto é deliberadamente
-- provisório — a Fase 2 do roadmap prevê migrar os consumidores para
-- `notifications` e parar esta dupla escrita.
-- Ver docs/superpowers/plans/2026-07-27-motor-automacao-fechar-mvp.md.
