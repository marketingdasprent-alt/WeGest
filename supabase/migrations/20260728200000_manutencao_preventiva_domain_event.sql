-- Item 7/13 da lista do chefe: "Alerta Plano de Manutenção preventiva".
-- Não existia nenhum conceito de plano de manutenção no sistema (só
-- manutenção reativa, viatura_reparacoes) — versão simples decidida com
-- o utilizador: 2 campos novos por viatura (próxima data e/ou próximo
-- km), editáveis na ficha (aba Dados Técnicos). O alerta dispara quando
-- QUALQUER um dos dois se aproxima — 15 dias (mesmo padrão dos outros)
-- ou 500 km antes (viaturas.km_atual já é mantido atualizado por
-- check-ins/movimentos/fecho de ticket).
...
begin
  ...
  select
    ...
    jsonb_build_object(
      ...
      'proxima_manutencao_data', v.proxima_manutencao_data,
      'proxima_manutencao_km', v.proxima_manutencao_km,
      'km_atual', v.km_atual
    ),
    'cron'
  from public.viaturas v
  where (v.is_vendida is null or v.is_vendida = false)
    and coalesce(v.status, 'disponivel') <> 'inativo'
    and (
      (v.proxima_manutencao_data is not null and v.proxima_manutencao_data <= current_date + 15)
      or (
        v.proxima_manutencao_km is not null
        and v.km_atual is not null
        and v.km_atual >= v.proxima_manutencao_km - 500
      )
    )
    and not exists (
      select 1 from public.domain_events e
      where e.entity_table = 'viaturas'
        and e.entity_id = v.id
        and e.event_type = 'viatura.manutencao_preventiva_expirando'
        and e.processed_at is null
    );
