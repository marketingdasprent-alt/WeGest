-- ⚠️ FICHEIRO RECUPERADO E RE-CARIMBADO (2026-08-28)
--
-- Esta migração foi aplicada em produção a 2026-08-26 e registada como
-- `20260826153601`. O ficheiro existia apenas no commit 941a6746
-- (`fix(integracoes): token Apify partilhado entre organizações`), que não está
-- em nenhum ramo actual, e com o carimbo `20260826160000` — diferente do que
-- produção registou.
--
-- Foi recuperado para o working tree com o carimbo de PRODUÇÃO, para que uma
-- reconstrução a partir das migrações produza a mesma linha em
-- `supabase_migrations.schema_migrations` que produção tem. O conteúdo é o
-- original, byte a byte, a partir daqui.
--
-- NOTA: o commit 941a6746 traz também `supabase/functions/apify-credenciais-partilhadas/`
-- e alterações a `src/components/admin/IntegracaoDialog.tsx` e
-- `src/components/admin/integracoes/types.ts` que continuam por integrar.
-- Estão fora do âmbito da reconciliação da cadeia de migrações — ver
