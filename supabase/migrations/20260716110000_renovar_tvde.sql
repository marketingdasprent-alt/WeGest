-- ============================================================
-- Renovação para TVDE — exactamente como rent-a-car
-- ============================================================
-- A renovação (fechar o mês com os km + abrir o mês seguinte por faturar)
-- deixa de ser exclusiva do rent-a-car e passa a aplicar-se também a TVDE,
-- com as mesmas características. Única diferença estrutural: contratos TVDE
-- podem viver SEM data_fim (contrato em aberto — todos os 189 ativos têm
-- is_longa_duracao=true e opção de renovação, mas 183 não têm data de fim).
-- Nesses, a 1.ª renovação arranca o ciclo: o período fecha HOJE e o novo mês
-- começa agora (now() → proxima_data_renovacao). A partir daí o contrato tem
-- data_fim e comporta-se exactamente como um rent-a-car de longa duração
-- (banner de renovações, "em atraso", km excedente, fecho do antigo).
--
-- Alterações à RPC renovar_contrato_renting (única mudança vs 20260716090000):
--   · o guard "sem data de fim" passa a exigir data_fim só fora de TVDE;
--   · v_inicio := COALESCE(v_old.data_fim, now()).
-- O resto (fecho do antigo como 'cancelado', km, DUA, km excedente, clones)
-- mantém-se intacto. O gate de regime no frontend (contratoRenovavel) é
-- alargado em paralelo no código TypeScript.
-- ============================================================
