-- Item 10/13 da lista do chefe: "Lembrete ao motorista sobre ficha
-- incompleta". "Ficha incompleta" já tem uma definição canónica no
-- frontend — src/lib/motoristaFichaCompleta.ts (CAMPOS_FICHA_OBRIGATORIOS:
-- nif, email, iban, telefone, documento_tipo, documento_numero,
-- documento_validade), usada em Motoristas.tsx / MotoristasFichaIncompleta.tsx
-- (visão do staff). Este item reutiliza exatamente os mesmos 7 campos,
-- mas lembra o PRÓPRIO motorista (não o staff) — scan diário, mesmo
-- padrão de emit_candidaturas_paradas_events() (item 3) e
-- emit_tickets_atrasados_events() (item 9).

create or replace function public.emit_motoristas_ficha_incompleta_events()
returns void
language plpgsql
security definer
...
begin
  ...
  select
    ...
    jsonb_build_object(
      'nome', m.nome,
      'campos_em_falta', to_jsonb(array_remove(array[
        case when m.nif is null or btrim(m.nif) = '' then 'NIF' end,
        case when m.email is null or btrim(m.email) = '' then 'Email' end,
        case when m.iban is null or btrim(m.iban) = '' then 'IBAN' end,
        case when m.telefone is null or btrim(m.telefone) = '' then 'Telefone' end,
        case when m.documento_tipo is null or btrim(m.documento_tipo) = '' then 'Tipo de documento' end,
        case when m.documento_numero is null or btrim(m.documento_numero) = '' then 'Nº documento' end,
        case when m.documento_validade is null then 'Validade documento' end
      ], null))
    ),
    'cron'
  ...
  where m.status_ativo = true
    ...
    and (
      m.nif is null or btrim(m.nif) = ''
      or m.email is null or btrim(m.email) = ''
      or m.iban is null or btrim(m.iban) = ''
      or m.telefone is null or btrim(m.telefone) = ''
      or m.documento_tipo is null or btrim(m.documento_tipo) = ''
      or m.documento_numero is null or btrim(m.documento_numero) = ''
      or m.documento_validade is null
    )
    and not exists (
      select 1 from public.domain_events e
      where e.entity_table = 'motoristas_ativos'
        and e.entity_id = m.id
        and e.event_type = 'motorista.ficha_incompleta'
        and e.processed_at is null
    );
end;
$$;

revoke all on function public.emit_motoristas_ficha_incompleta_events() from public, anon, authenticated;
grant execute on function public.emit_motoristas_ficha_incompleta_events() to service_role;

select cron.schedule(
  'automation-emit-motoristas-ficha-incompleta-diario',
  '0 8 * * *',
