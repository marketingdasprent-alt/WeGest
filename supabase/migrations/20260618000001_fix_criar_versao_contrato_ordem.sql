-- Fix: criar_versao_contrato_renting marcava o contrato antigo como substituído
-- DEPOIS de inserir o novo. O índice único parcial
-- uq_contratos_renting_reserva_id_active (WHERE deleted_at IS NULL AND substituido_em IS NULL)
-- via momentaneamente dois registos activos para a mesma reserva_id → erro 23505.
-- Solução: marcar primeiro o antigo como substituído (passo 1), só depois inserir o novo (passo 2).
...
BEGIN
  ...
  --    Isto liberta o slot no índice único parcial
  --    uq_contratos_renting_reserva_id_active (WHERE deleted_at IS NULL AND substituido_em IS NULL)
  --    e evita o erro 23505 que ocorria quando o INSERT tentava ocupar
  --    o mesmo reserva_id que o registo antigo ainda activo.
...
COMMENT ON FUNCTION public.criar_versao_contrato_renting(uuid, text) IS
  ...
  'Fix 2026-06-18: marcar substituido_em ANTES do INSERT para evitar 23505.';
