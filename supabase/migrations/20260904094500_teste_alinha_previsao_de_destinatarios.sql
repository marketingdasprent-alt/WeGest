-- ============================================================================
-- A pré-visualização do teste deixa de prometer admins que já não recebem
-- ============================================================================
--
-- `testar_regra_automacao` devolve, à parte do envio, a lista de quem
-- receberia numa execução normal — e essa lista é uma CÓPIA de leitura da
-- resolução de destinatários do executor. Quando 20260904093000 tirou o
-- fallback de admin das acções de email, esta cópia ficou para trás: o toast
-- do botão "Testar" continuava a dizer "Iria também para: ... e mais 10".
--
-- Duas cópias da mesma regra divergiram, que é o risco assumido quando se
-- escolheu não tocar no executor. Fica alinhada; se voltar a divergir, o
-- caminho é extrair a resolução para uma função partilhada.
-- ============================================================================
  ...
  if v_estrategia = 'motorista' then
    ...
    where (
      ...
      v_gestor_user_id is null and (
        -- Mesmo guarda que o executor: uma acção de email não arrasta
        -- os administradores da organização (ver 20260904093000).
