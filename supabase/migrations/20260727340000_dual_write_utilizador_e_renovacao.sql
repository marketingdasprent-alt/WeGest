-- Fix: utilizador.criado e contrato_renting.renovacao_proxima nunca
-- entraram na dupla escrita (v_tipo_legado só mapeava os 5 tipos
-- originais) — as notificações ficavam só em `notifications` (tabela
-- nova), que não tem nenhum consumidor visível para o utilizador final.
-- Resultado: 462 notificações já geradas para estes 2 tipos, ZERO
-- visíveis no sino/popup real de qualquer gestor. Corrige alargando o
-- mapeamento e o CHECK, com link direto para o contrato/página de
-- utilizadores.
...
alter table public.notificacoes add constraint notificacoes_tipo_check
  check (tipo = any (array[
    ...
    'cobranca_gerada', 'utilizador_criado', 'contrato_renting_renovacao_proxima'
  ]::text[]));
...
-- RLS: os 2 tipos novos partilham o mesmo ramo (destinatario_id = auth.uid())
-- já usado pelos outros 7.
