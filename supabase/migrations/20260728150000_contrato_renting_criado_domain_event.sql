-- Item 2/13 da lista do chefe: "Contrato de Aluguer criado" — aviso
-- quando nasce um novo contrato de renting (rent-a-car ou TVDE; "Aluguer"
-- não é uma tabela à parte, é o termo usado para contratos_renting em
-- qualquer regime). Espelha fn_contrato_cobrancas_domain_event()
-- (20260727180000_cobranca_gerada_domain_event.sql): trigger direto para
-- domain_events, sem passar por cron.
--
-- Guardado por contrato_anterior_id is null — renovações
-- (renovar_contrato_renting) e novas versões por edição material
-- (criar_versao_contrato_renting) criam uma nova linha com
-- contrato_anterior_id preenchido; só um contrato genuinamente novo tem
-- este campo vazio.
...
drop trigger if exists trg_contratos_renting_criado_domain_event on public.contratos_renting;
create trigger trg_contratos_renting_criado_domain_event
  after insert on public.contratos_renting
  for each row
  execute function public.fn_contratos_renting_criado_domain_event();

-- Regra + template novos (seed_automacao_defaults) — sem cargo por
-- omissão, a organização escolhe quem recebe no ecrã Configurar.
...
-- Dual-write para a bell antiga (mesmo padrão dos outros event_types).
  ...
  and (
    ...
    or (
      tipo = any (array[
        ...
        'sistema_job_falhou', 'contrato_renting_criado'
      ])
...
-- execute_automation_runs(): mapeia o novo event_type para o tipo legado
-- (dual-write). v_link já cobre entity_table='contratos_renting'.
