...
function explicarErroBolt(erro: unknown): string {
  if (erro instanceof BoltApiError) {
    switch (erro.codigo) {
      case 498810:
        return (
          'A Bolt recusou o acesso a esta empresa (COMPANY_NOT_ALLOWED). ' +
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const fase =
      fasePedida === 'viagens' || fasePedida === 'agregar'
    ...
    if (!clientId || !clientSecret) {
      const mensagem = `A integração "${config.nome}" está em modo oauth mas não tem client_id e/ou client_secret.`;
    ...
    if (periodo_inicio || periodo_fim) {
      if (periodo_inicio && periodo_fim) {
        ...
        if (!inicio || !fim) {
          return jsonError(
            'periodo_inicio/periodo_fim têm de ser datas no formato YYYY-MM-DD.',
            400
          );
        }
    ...
    const filtroTemporal: FiltroTemporal =
      typeof filtro_temporal === 'string' &&
      FILTROS_TEMPORAIS.includes(filtro_temporal as FiltroTemporal)
    ...
    if (
      companyIdPedido !== undefined &&
      companyIdPedido !== null &&
      String(companyIdPedido).trim() !== ''
    ) {
    ...
    if (companyId === null) {
      ...
      if (empresasDaCredencial.length === 1) {
        ...
        const mensagem = `As credenciais da integração "${config.nome}" não estão associadas a nenhuma empresa na Bolt.`;
        await registarLog('error', mensagem, { erros: 1, periodo: semana.periodo });
        return jsonError(mensagem, 409);
      } else {
        const mensagem =
    ...
    if (fase === 'agregar') {
      const semanaBase =
        typeof semanaInicioPedida === 'string' && semanaInicioPedida
          ? semanaInicioPedida
          : semana.inicio;
    ...
    for (const linha of gravaResumos ? agregado.linhas : []) {
      ...
      console.error(
        `[bolt-sync-semana] merge falhou para a chave ${linha.chave}: ${error.message}`
      );
    }
    ...
    if (resumosComErro > 0 && resumosGravados > 0) {
      avisos.push(
        `${resumosComErro} motoristas não gravados (${errosResumo[0] ?? 'erro desconhecido'})`
      );
    }
    if (agregado.ordens_ignoradas > 0) {
      avisos.push(
        `${agregado.ordens_ignoradas} viagens sem motorista identificável (sem driver_uuid nem nome)`
      );
    }
    if (viagensComErro > 0) {
      avisos.push(
        `${viagensComErro} viagens não gravadas em bolt_viagens (${errosViagens[0] ?? 'erro desconhecido'})`
      );
    }
    ...
    if (resumosGravados > 0) {
      const { error } = await supabase
        ...
        .eq('id', integracao_id);
      if (error)
        console.warn(`[bolt-sync-semana] falha a actualizar ultimo_sync: ${error.message}`);
    }
