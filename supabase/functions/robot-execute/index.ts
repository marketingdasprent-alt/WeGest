...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (authMode === "cookies") {
      ...
      const portalEmail = config.robot_portal_email ||
        (authMode !== "oauth" ? config.client_id : null);
      const portalPassword = config.robot_portal_password ||
        (authMode !== "oauth" ? config.client_secret : null);
