...
import { verifyRobotWebhookSignature } from "../_shared/integracoes/robotWebhookSecurity.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const hasValidWebhookSignature = await verifyRobotWebhookSignature(
      integracaoId,
      signature,
      SERVICE_ROLE_KEY,
    );
    ...
    if (!isInternal && !hasValidWebhookSignature) {
    ...
    const { data: config, error: configError } = await supabase
      ...
      .select("id, nome, plataforma, robot_target_platform, org_id")
      .eq("id", integracaoId)
    ...
    console.log(
      `robot-webhook received for integration ${integracaoId} (${config.robot_target_platform}). Keys: [${
        payloadKeys.join(", ")
      }]`,
    );
    ...
    for (const [k, v] of Object.entries(payload)) {
      if (typeof v === "string") console.log(`  key="${k}" length=${v.length}`);
        ...
        console.log(`  key="${k}" array length=${v.length}`);
      }
    }
    ...
    const findCsvInPayload = (obj: any): string | null => {
      console.log(
        "robot-webhook: Searching for CSV in payload keys:",
    ...
    if (boltCsvContent) {
      console.log(
        "robot-webhook: Bolt CSV data detected, forwarding to bolt-import-csv",
      );
    ...
    if (hasBpData) {
      console.log(
        "robot-webhook: BP fuel CSV data detected, forwarding to bp-import-csv",
      );
    ...
    if (config.robot_target_platform === "repsol") {
      console.log(
        "robot-webhook: Repsol integration detected, forwarding to repsol-import-csv",
      );

      const knownData = payload.combustivel_csv ||
        payload.dados_csv_combustivel ||
        payload.movimentos ||
        payload.data ||
        findCsvInPayload(payload);
      const fallbackArray = !knownData
        ? (Object.values(payload).find((v) => Array.isArray(v)) as
          | any[]
          | undefined)
        : undefined;
    ...
    if (config.robot_target_platform === "edp") {
      const edpData = payload.combustivel_csv ||
        payload.dados_csv_combustivel ||
        payload.data ||
        findCsvInPayload(payload);
      if (edpData) {
        console.log(
          "robot-webhook: EDP fuel data detected, forwarding to edp-import-csv",
        );
        ...
        return new Response(
          JSON.stringify({
            success: resp.ok,
            message: resp.ok
              ? "Dados EDP processados com sucesso"
              : "Erro ao processar dados EDP",
            integracao_id: integracaoId,
            import_result: result,
          }),
          {
    ...
    const uberPagamentosCsv = payload.pagamentos_csv ||
      payload.dados_csv_pagamentos;
    ...
    const uberFallbackCsv =
      config.robot_target_platform === "uber" && !uberPagamentosCsv &&
        !uberViagensCsv
    ...
    if (hasUberData) {
      console.log(
        "robot-webhook: Uber CSV data detected, forwarding to uber-import-reports",
      );

      // Extract original filename so uber-import-reports can parse the period from it
      const nomeOriginal: string = payload.nome_ficheiro || payload.filename ||
      ...
      if (uberPagamentosCsv) {
        ...
        forwardBody.nome_pagamentos = payload.nome_pagamentos || nomeOriginal ||
          "pagamentos.csv";
      }
      if (uberViagensCsv) {
        forwardBody.viagens_csv = uberViagensCsv;
        forwardBody.nome_viagens = payload.nome_viagens || nomeOriginal ||
          "viagens.csv";
      }
      ...
      console.log(
        `robot-webhook: Uber forward — pagamentos=${!!forwardBody
          .pagamentos_csv}, viagens=${!!forwardBody
          .viagens_csv}, nome="${nomeOriginal}"`,
      );
    ...
    if (config.robot_target_platform === "viaverde") {
      const viaVerdeCsv = payload.dados_csv ||
        payload.csv_content ||
        payload.dados_csv_viaverde ||
        findCsvInPayload(payload);
      const viaVerdeTransacoes = Array.isArray(payload.transacoes)
        ? payload.transacoes
        : null;

      if (viaVerdeCsv || viaVerdeTransacoes) {
        console.log(
          "robot-webhook: Via Verde data detected, forwarding to via-verde-import",
        );

        const resp = await fetch(
          `${SUPABASE_URL}/functions/v1/via-verde-import`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${SERVICE_ROLE_KEY}`,
    ...
    if (config.robot_target_platform === "viaverde") {
      await supabase
        ...
        .update({
          ...
          completed_at: new Date().toISOString(),
          error_message:
            "Formato de dados não reconhecido. Nenhum CSV detetado.",
        })
        .eq("integracao_id", integracaoId)
        .eq("status", "running");
    }
