...
Deno.test(
  'TVDE em período parcial (3 dias, não semana inteira) → rateio por 7 dias fixo, não pelo tamanho do período',
  () => {
...
Deno.test(
  'sem motoristaId no condutor → custoMotorista é null, receitaViatura continua a calcular',
  () => {
...
Deno.test(
  'com motoristaId → custoMotorista.custoAluguer é igual ao receitaViatura.receitaAluguer',
  () => {
    const r = buildWeeklyContractSummary({
      ...semanaBase,
      contrato: contratoTvdeSemanaInteira,
...
Deno.test(
  'bolt/uber e categorias de motorista_financeiro agregam certo do lado do motorista',
  () => {
    const r = buildWeeklyContractSummary({
      ...semanaBase,
      contrato: contratoTvdeSemanaInteira,
      ...
      motoristaFinanceiro: [
        ...
        // O fecho semanal não tem coluna própria para slot: dobra-se dentro de
        // despesaOutros (25 + 60 = 85). No resumo individual do motorista e na
        // lista de Contas/Resumo, esta categoria tem linha própria — ver
        // movimentosMotorista.ts e useContasResumoSemana.ts.
        { tipo: 'debito', categoria: 'slot_mensal', valor: 60 },
    ...
    assertEquals(r.custoMotorista?.despesaOutros, 85);
  }
);
