/** As respostas têm dados pessoais: nunca registar corpos, credenciais ou tokens. */
...
/** Limites documentados por endpoint; os restantes usam o limite Pager. */
...
// Só `order_price` é garantido pela spec; o restante payload Bolt é opcional.
...
/** Diagnóstico: devolve a vida anunciada pela Bolt, não a margem de renovação. */
...
export async function callBolt<T>(
  ...
  throw ultimoErro ?? new Error(`[bolt] ${operacao}: esgotadas as ${MAX_TENTATIVAS} tentativas`);
}
...
export async function paginar<T>(
  ...
  // Com total declarado, página curta é falha transitória, não fim dos dados.
  ...
  // O ramo paralelo não pode alterar o total usado para calcular offsets.
  const lerPagina = async (posicao: number): Promise<{ itens: T[]; total: number | undefined }> => {
    let itens: T[] = [];
  ...
  // Com o total, os offsets são independentes e podem ser lidos em paralelo.
  if (concorrencia > 1 && typeof totalDeclarado === 'number' && offsetInicial === 0) {
    ...
    // A agregação exige ordem reproduzível entre execuções.
...
function verificarTotal(
  ...
  // Com offset inicial, o total declarado abrange um intervalo diferente.
